...
def get_date_range(days: int = 400) -> tuple[str, str]:
...
def get_close_prices(tickers: list[str] = TICKERS, days: int = 400) -> pd.DataFrame:
    """
    Download historical close prices for given tickers.
    ...
    """
    start_str, end_str = get_date_range(days)

    print(
        f"Downloading data for {len(tickers)} tickers from {start_str} to {end_str}...")
    ...
    # Handle different return types from yfinance depending on number of tickers
    if isinstance(data, pd.DataFrame) and "Close" in data.columns:
        # Multi-level column index or just "Close" column
        try:
            result = data["Close"]
        except KeyError:
            result = data
    ...
    # Drop columns with any missing values to ensure data consistency
